...
<?php 
    ...
    if($this->input->post('msg_subject') && $this->input->post('msg_subject') != '' && $this->input->post('msg_subject') != null){
        ...
        $userd  =  $this->AppUser->getUserDetails($this->input->post('msg_recievers'));
        ...
        if(!count($userd )){
         ...
         // return;
          ...
          $toPass = array();
          $toPass   = $this->input->post(null,true);
          $toPass['msg_recievers']   = $userd['lu_id'];
          $sResponse =  $this->MyLibrary->sendMessage($toPass);
        }
